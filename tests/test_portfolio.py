...
class TestPortfolioOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.optimizer = PortfolioOptimizer(n_assets=self.n_assets, lookback_window=self.lookback_window)
        ...
        self.volatility_data = np.vstack([self.volatility_data[0:1], self.volatility_data])
    ...
    def test_model_architecture(self):
        ...
        # Check input shapes
        self.assertEqual(self.optimizer.model.inputs[0].shape[1:], (self.lookback_window, self.n_assets))  # price_input
        self.assertEqual(self.optimizer.model.inputs[1].shape[1:], (self.lookback_window, self.n_assets))  # vol_input
        self.assertEqual(self.optimizer.model.inputs[2].shape[1:], (self.lookback_window, 5))  # macro_input
        self.assertEqual(self.optimizer.model.inputs[3].shape[1:], (self.n_assets,))  # current_weights
    ...
    def test_portfolio_loss(self):
        ...
        y_pred = tf.constant(np.random.dirichlet(np.ones(self.n_assets), size=batch_size), dtype=tf.float32)
        y_true = tf.constant(np.random.normal(0.001, 0.01, (batch_size, self.n_assets)), dtype=tf.float32)
    ...
    def test_preprocess_data(self):
        ...
        self.assertEqual(X_price.shape, (expected_samples, self.lookback_window, self.n_assets))
        self.assertEqual(X_vol.shape, (expected_samples, self.lookback_window, self.n_assets))
        ...
        for i in range(expected_samples):
            expected_returns = self.price_data[i + self.lookback_window] / self.price_data[i + self.lookback_window - 1] - 1
