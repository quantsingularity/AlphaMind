...
class ModelComparison:
    ...
    def statistical_comparison(
        ...
        for model_name, model in self.models.items():
            ...
            t_test_result = self.statistical_tests.paired_t_test(
                baseline, model, X, y
            )
    ...
    def plot_comparison(
        ...
        metric_values = {model: results[metric] for model, results in self.results.items()}
        metric_std = {model: results.get(f"{metric}_std", 0) for model, results in self.results.items()}
    ...
    def plot_metric_heatmap(
        ...
        for i in range(len(results_df.index)):
            for j in range(len(metric_cols)):
                ax.text(
                    j, i, f"{results_df.iloc[i, j]:.4f}",
                    ha="center", va="center",
                    color="white" if results_df.iloc[i, j] > results_df[metric_cols[j]].mean() else "black"
                )
...
class StatisticalTests:
    ...
    def diebold_mariano_test(
        ...
        if loss_fn is None:
            ...
            d = np.array([loss_fn(y_true[i], y_pred1[i]) - loss_fn(y_true[i], y_pred2[i]) for i in range(len(y_true))])
    ...
    def model_confidence_set(
        ...
        while len(model_set) > 1:
            ...
            for (model_i, model_j), diff in loss_diffs.items():
                t_stats[(model_i, model_j)] = np.mean(diff) / (np.std(diff) / np.sqrt(len(diff)))
            ...
            for _ in range(B):
                ...
                for (model_i, model_j), diff in boot_diffs.items():
                    boot_t_stats[(model_i, model_j)] = np.mean(diff) / (np.std(diff) / np.sqrt(len(diff)))
