...
def calculate_metrics(
    ...
    if task_type == "regression":
        ...
        if len(np.unique(y_true)) <= 2:
            metrics["accuracy"] = accuracy_score(y_true, np.round(y_pred), sample_weight=sample_weights)
            metrics["precision"] = precision_score(y_true, np.round(y_pred), sample_weight=sample_weights)
            metrics["recall"] = recall_score(y_true, np.round(y_pred), sample_weight=sample_weights)
            metrics["f1"] = f1_score(y_true, np.round(y_pred), sample_weight=sample_weights)
            
            # AUC and log loss require probability estimates
            if np.all((y_pred >= 0) & (y_pred <= 1)):
                metrics["auc"] = roc_auc_score(y_true, y_pred, sample_weight=sample_weights)
                metrics["log_loss"] = log_loss(y_true, y_pred, sample_weight=sample_weights)
        
        # For multi-class classification
        else:
            metrics["accuracy"] = accuracy_score(y_true, np.argmax(y_pred, axis=1), sample_weight=sample_weights)
            metrics["precision_macro"] = precision_score(y_true, np.argmax(y_pred, axis=1), average='macro', sample_weight=sample_weights)
            metrics["recall_macro"] = recall_score(y_true, np.argmax(y_pred, axis=1), average='macro', sample_weight=sample_weights)
            metrics["f1_macro"] = f1_score(y_true, np.argmax(y_pred, axis=1), average='macro', sample_weight=sample_weights)
            
    elif task_type == "ranking":
        # Spearman rank correlation
        metrics["spearman_corr"] = pd.Series(y_pred).corr(pd.Series(y_true), method='spearman')
...
def information_coefficient(
    ...
    if rank:
        # Spearman rank correlation
        return pd.Series(factor_values).corr(pd.Series(forward_returns), method='spearman')
    else:
        # Pearson correlation
