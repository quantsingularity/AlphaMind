...
class ValidationReport:
    ...
    def add_feature_importance(
        ...
        self.sections["feature_importance"] = importance_values.to_dict(orient="records")
    
    def add_residual_analysis(
        self,
        y_true: np.ndarray,
        y_pred: np.ndarray
    ) -> None:
    ...
    def _generate_markdown_report(
        ...
        if self.sections["diagnostics"]:
            ...
            if "confusion_matrix" in self.sections["diagnostics"]:
                ...
                for row in cm["matrix"]:
                    report.write(" ".join(f"{x:.4f}" if cm["normalized"] else f"{x}" for x in row) + "\n")
                report.write("```\n\n")
            ...
            if "precision_recall_curve" in self.sections["diagnostics"]:
                ...
                report.write(f"- **Average Precision:** {pr['average_precision']:.4f}\n\n")
            ...
            if "learning_curve" in self.sections["diagnostics"]:
                ...
                report.write("| Train Size | Train Score | Train Std | Test Score | Test Std |\n")
                report.write("|----------:|-----------:|---------:|----------:|--------:|\n")
            ...
            if "calibration_curve" in self.sections["diagnostics"]:
                ...
                for i in range(len(cc["prob_pred"])):
                    report.write(f"| {cc['prob_pred'][i]:.4f} | {cc['prob_true'][i]:.4f} |\n")
        ...
        for section_name, content in self.sections.items():
            if section_name not in ["model_info", "performance_metrics", "cross_validation", 
                                   "feature_importance", "diagnostics", "warnings"]:
    ...
    def plot_residuals(
        ...
        """
        if self.task_type != "regression":
            raise ValueError("Residual analysis is only applicable for regression models")
    ...
    def plot_confusion_matrix(
        ...
        """
        if self.task_type != "classification":
            raise ValueError("Confusion matrix is only applicable for classification models")
        ...
        cbar.ax.set_ylabel("Count" if not cm["normalized"] else "Proportion", rotation=-90, va="bottom")
        ...
        # Add text to cells
        thresh = (np.array(cm["matrix"]) > 0.5).astype(int) if cm["normalized"] else np.max(cm["matrix"]) / 2
        for i in range(len(cm["matrix"])):
            for j in range(len(cm["matrix"][i])):
                ax.text(j, i, f"{cm['matrix'][i][j]:.2f}" if cm["normalized"] else f"{cm['matrix'][i][j]}",
                        ha="center", va="center",
    ...
    def plot_roc_curve(
        ...
        # Plot ROC curve
        ax.plot(roc["fpr"], roc["tpr"], lw=2, label=f'ROC curve (AUC = {roc["auc"]:.4f})')
    ...
    def plot_precision_recall_curve(
        ...
        """
        if self.task_type != "classification":
            raise ValueError("Precision-recall curve is only applicable for classification models")
    ...
    def plot_calibration_curve(
        ...
        """
        if self.task_type != "classification":
            raise ValueError("Calibration curve is only applicable for classification models")
