...
class SEC8KMonitor:
    ...
    def __init__(self, tickers: List[str], max_retries: int = 3, retry_delay: int = 5):
        ...
        self.dl = Downloader(company_name="AlphaMind", email_address="alphamind@example.com")
    ...
    def download_filings(self, ticker: str, filing_type: str = "8-K", 
                         count: int = 10) -> bool:
        """
        ...
        while attempts < self.max_retries:
            try:
                ...
                logger.info(f"Successfully downloaded {filing_type} filings for {ticker}")
                return True
            except Exception as e:
                attempts += 1
                logger.warning(f"Attempt {attempts}/{self.max_retries} failed for {ticker}: {e}")
                if attempts < self.max_retries:
                    logger.info(f"Retrying in {self.retry_delay} seconds...")
                    time.sleep(self.retry_delay)
                else:
                    logger.error(f"Failed to download {filing_type} filings for {ticker} after {self.max_retries} attempts")
                    return False
    ...
    def process_filing(self, filing) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Processed filing from {filing.date} with sentiment: {sentiment}")
            return result
    ...
    def _calculate_sentiment_with_retry(self, text: str) -> str:
        ...
        while attempts < self.max_retries:
            try:
                ...
                attempts += 1
                logger.warning(f"Sentiment analysis attempt {attempts}/{self.max_retries} failed: {e}")
                if attempts < self.max_retries:
                    logger.info(f"Retrying sentiment analysis in {self.retry_delay} seconds...")
                    time.sleep(self.retry_delay)
                else:
                    logger.error(f"Failed to calculate sentiment after {self.max_retries} attempts")
                    return "unknown"
    ...
    def _calculate_sentiment(self, text: str) -> str:
        ...
        if len(text) > 512:
            logger.debug(f"Truncating text from {len(text)} to 512 characters for sentiment analysis")
            text = text[:512]
        ...
        if self.sentiment_model is None:
            try:
                ...
                self.sentiment_model = pipeline("text-classification", model="ProsusAI/finbert")
