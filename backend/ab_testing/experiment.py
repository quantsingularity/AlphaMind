...
class Experiment:
    ...
    def add_result(
        ...
        self.results[metric][variant].append({
            "value": value,
            "timestamp": timestamp
        })
    ...
    def add_results_batch(
        ...
        timestamp_col : str, optional
            ...
            If None, uses current time for all results.
        """
    ...
    def get_results(
        ...
        if variant and not any(variant in self.results.get(m, {}) for m in self.results):
    ...
    def load(
        ...
        experiment.creation_date = datetime.datetime.fromisoformat(data["creation_date"])
...
class ExperimentGroup:
    ...
    def load(
        ...
        if "experiments" in data and not load_experiments:
            for exp_id, exp_data in data["experiments"].items():
                ...
                # Set attributes
                experiment.id = exp_data["id"]
                experiment.variants = exp_data["variants"]
                
                if exp_data["start_date"]:
                    experiment.start_date = datetime.datetime.fromisoformat(exp_data["start_date"])
                
                if exp_data["end_date"]:
                    experiment.end_date = datetime.datetime.fromisoformat(exp_data["end_date"])
                
                experiment.status = ExperimentStatus(exp_data["status"])
                experiment.creation_date = datetime.datetime.fromisoformat(exp_data["creation_date"])
                experiment.metadata = exp_data["metadata"]
                
                group.add_experiment(experiment)
        
        # Load individual experiment files if requested
        elif load_experiments:
            directory = os.path.dirname(filepath)
            experiments_dir = os.path.join(directory, f"{group.id}_experiments")
