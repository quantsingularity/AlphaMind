    ...
    def run(
        ...
        if alternative == "two-sided":
            ci_lower, ci_upper = stats.t.interval(
                ...
                scale=np.sqrt(control_std**2 / control_n + treatment_std**2 / treatment_n)
            )
        ...
        self.results = {
            ...
            "ci_lower": ci_lower,
            "ci_upper": ci_upper,
            "relative_difference": (treatment_mean - control_mean) / control_mean if control_mean != 0 else np.nan
        }
...
class MannWhitneyU(StatisticalTest):
    ...
    def run(
        ...
        self.results = {
            ...
            "relative_difference": (treatment_median - control_median) / control_median if control_median != 0 else np.nan
        }
...
class BayesianABTest(StatisticalTest):
    ...
    def run(
        ...
        if set(np.unique(control)).issubset({0, 1}) and set(np.unique(treatment)).issubset({0, 1}):
            # Beta-binomial model
            ...
            self.results = {
                ...
                "ci_lower": ci_lower,
                "ci_upper": ci_upper,
                "relative_improvement": expected_improvement / control_mean if control_mean != 0 else np.nan
            }
            ...
            # Draw samples from posterior distributions
            control_samples = np.random.normal(control_mean_posterior, control_std_posterior, n_samples)
            treatment_samples = np.random.normal(treatment_mean_posterior, treatment_std_posterior, n_samples)
    ...
    def benjamini_hochberg(
        ...
        for i in range(n_tests - 2, -1, -1):
            corrected_sorted_p_values[i] = min(
                corrected_sorted_p_values[i],
                corrected_sorted_p_values[i + 1]
            )
