...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ddpg_trading.log"),
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        # Convert to tensors
        states = torch.FloatTensor([self._flatten_dict_state(e.state) for e in experiences])
        ...
        next_states = torch.FloatTensor([self._flatten_dict_state(e.next_state) for e in experiences])
...
class DDPGAgent:
    ...
    def __init__(self, env, config=None):
        ...
        self.actor = Actor(self.state_dim, self.action_dim, 
                          hidden_dims=self.config["actor_hidden_dims"]).to(self.device)
        self.critic = Critic(self.state_dim, self.action_dim, 
                            hidden_dims=self.config["critic_hidden_dims"]).to(self.device)
        ...
        self.results_dir = os.path.join("results", f"ddpg_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        os.makedirs(self.results_dir, exist_ok=True)
        
        logger.info(f"DDPG Agent initialized with state_dim={self.state_dim}, action_dim={self.action_dim}")
    ...
    def train(self, max_episodes=None, max_steps=None):
        ...
        logger.info(f"Starting training for {max_episodes} episodes, max {max_steps} steps per episode")
        ...
        for episode in range(1, max_episodes + 1):
            ...
            if episode % self.config["log_interval"] == 0:
                ...
                logger.info(f"Episode {episode}/{max_episodes} | Avg Reward: {avg_reward:.2f} | Buffer Size: {len(self.replay_buffer)}")
            ...
            if episode % self.config["eval_interval"] == 0:
                ...
                logger.info(f"Evaluation after episode {episode}: Avg Reward = {eval_reward:.2f}")
...
class TradingGymEnv(gym.Env):
    ...
    def __init__(self, data_stream, features=None, window_size=10, transaction_cost=0.001, 
                 reward_scaling=1.0, max_steps=252):
        """
        ...
        self.observation_space = spaces.Dict({
            "prices": spaces.Box(
                low=-np.inf, high=np.inf, shape=(self.num_assets, window_size), dtype=np.float32
            ),
            "volumes": spaces.Box(
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            print(f"Current Return: {self.episode_returns[-1] if self.episode_returns else 0:.4f}")
            print("-" * 50)
...
class BacktestEngine:
    ...
    def run(self, episodes=1, render=False):
        ...
        for episode in range(1, episodes + 1):
            ...
            self.logger.info(f"Episode {episode}/{episodes} | Final Value: ${final_value:.2f} | Sharpe: {episode_sharpe:.2f}")
        
        # Calculate aggregate metrics
        self._calculate_metrics(all_portfolio_values, all_returns, all_actions)
        
        # Log final results
        self.logger.info(f"Backtest completed | Total Return: {self.total_return:.2%} | Sharpe: {self.sharpe_ratio:.2f} | Max Drawdown: {self.max_drawdown:.2%}")
...
class HyperparameterTuner:
    ...
    def __init__(self, env_creator, param_grid, n_trials=10, episodes_per_trial=5, max_steps=100):
        """
        ...
        # Create results directory
        self.results_dir = os.path.join("results", f"tuning_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        os.makedirs(self.results_dir, exist_ok=True)
    ...
    def run(self):
        ...
        for trial in range(1, self.n_trials + 1):
            ...
            # Train agent
            rewards = agent.train(
                max_episodes=self.episodes_per_trial,
            ...
            self.logger.info(f"Trial {trial} completed | Eval Reward: {eval_reward:.2f}")
        ...
        self.logger.info(f"Tuning completed | Best config: {best_config} | Reward: {best_reward:.2f}")
    ...
    def _plot_results(self):
        ...
        plt.scatter([trials[best_idx]], [rewards[best_idx]], color='red', s=100, label='Best')
        plt.legend()
