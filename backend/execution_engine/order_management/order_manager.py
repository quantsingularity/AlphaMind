...
class Order:
    ...
    def update_status(self, new_status: OrderStatus) -> None:
        ...
        logger.info(f"Order {self.order_id} status changed from {old_status.value} to {new_status.value}")
    ...
    def add_fill(self, fill: OrderFill) -> None:
        ...
        if abs(self.filled_quantity - self.quantity) < 1e-6:  # Account for floating point precision
            self.update_status(OrderStatus.FILLED)
        elif self.filled_quantity > 0:
            self.update_status(OrderStatus.PARTIALLY_FILLED)
            
        self.updated_at = datetime.datetime.now()
        logger.info(f"Added fill {fill.fill_id} to order {self.order_id}, filled quantity: {self.filled_quantity}/{self.quantity}")
...
class OrderValidator:
    ...
    def validate(self, order: Order) -> List[OrderValidationError]:
        ...
        for rule in self.validation_rules:
            try:
                ...
                all_errors.append(OrderValidationError(
    ...
    def _validate_price_fields(self, order: Order) -> List[OrderValidationError]:
        ...
        if order.order_type in [OrderType.LIMIT, OrderType.STOP_LIMIT, OrderType.ICEBERG]:
...
class OrderManager:
    ...
    def create_order(self, **kwargs) -> Order:
        ...
        logger.info(f"Created order {order.order_id} for {order.quantity} {order.instrument_id}")
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if order.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED, OrderStatus.EXPIRED, OrderStatus.ERROR]:
    ...
    def add_fill(self, order_id: str, fill: OrderFill) -> bool:
        ...
        if order.status in [OrderStatus.CANCELLED, OrderStatus.REJECTED, OrderStatus.EXPIRED, OrderStatus.ERROR]:
            logger.warning(f"Cannot add fill to order {order_id} with status {order.status.value}")
            return False
            
        # Check if fill would exceed order quantity
        if order.filled_quantity + fill.quantity > order.quantity + 1e-6:  # Account for floating point precision
    ...
    def get_orders_by_instrument(self, instrument_id: str) -> List[Order]:
        ...
        Returns:
            List of matching Order objects
        """
        return [order for order in self.orders.values() if order.instrument_id == instrument_id]
