...
class ReconnectionManager:
    ...
    def start(self, 
              ...
              on_give_up_callback: Optional[Callable[[], None]] = None) -> None:
        """
        ...
        """
        if self.is_running:
            logger.warning(f"Reconnection manager for {self.connection_id} is already running")
            return
    ...
    def connection_lost(self) -> None:
        ...
        """
        if not self.is_running:
            logger.warning(f"Reconnection manager for {self.connection_id} is not running")
            return
            
        self.disconnect_time = datetime.datetime.now()
        logger.info(f"Connection lost for {self.connection_id}, initiating reconnection")
    ...
    def connection_established(self) -> None:
        ...
        if self.disconnect_time:
            downtime = (self.last_connection_time - self.disconnect_time).total_seconds()
            self.stats.total_downtime += downtime
            logger.info(f"Connection {self.connection_id} restored after {downtime:.2f}s downtime")
            self.disconnect_time = None
        ...
        logger.info(f"Connection established for {self.connection_id}, reconnection state reset")
    ...
    def _attempt_reconnection(self) -> None:
        ...
        if self.state == ReconnectionState.CIRCUIT_OPEN:
            if datetime.datetime.now() < self.circuit_break_until:
                logger.info(f"Circuit breaker open for {self.connection_id}, skipping reconnection attempt")
                return
            else:
                logger.info(f"Circuit breaker timeout elapsed for {self.connection_id}, resetting")
                self.state = ReconnectionState.IDLE
                self.stats.consecutive_failures = 0
        
        # Check if max attempts has been reached
        if self.config.max_attempts > 0 and self.stats.total_attempts >= self.config.max_attempts:
            logger.warning(f"Maximum reconnection attempts ({self.config.max_attempts}) reached for {self.connection_id}")
        ...
        logger.info(f"Attempting reconnection for {self.connection_id} (attempt {self.stats.total_attempts})")
        ...
        try:
            ...
            if success:
                ...
                if self.stats.successful_reconnects == 1:
                    ...
                    self.stats.average_reconnect_time = (
                        (self.stats.average_reconnect_time * (self.stats.successful_reconnects - 1) + elapsed) / 
                        self.stats.successful_reconnects
                    )
                
                logger.info(f"Reconnection successful for {self.connection_id} in {elapsed:.2f}s")
            ...
            logger.error(f"Error during reconnection attempt for {self.connection_id}: {str(e)}")
            self._handle_reconnection_failure()
    ...
    def _handle_reconnection_failure(self) -> None:
        ...
        logger.warning(f"Reconnection failed for {self.connection_id} (consecutive failures: {self.stats.consecutive_failures})")
        
        # Check if circuit breaker should be triggered
        if self.stats.consecutive_failures >= self.config.circuit_break_threshold:
            self.state = ReconnectionState.CIRCUIT_OPEN
            self.circuit_break_until = datetime.datetime.now() + datetime.timedelta(seconds=self.config.circuit_break_timeout)
            
            logger.warning(f"Circuit breaker triggered for {self.connection_id}, pausing reconnection attempts until {self.circuit_break_until}")
            ...
            actual_delay = max(self.config.initial_delay, actual_delay)  # Ensure delay is not less than initial
            
            logger.info(f"Backing off for {actual_delay:.2f}s before next reconnection attempt for {self.connection_id}")
            
            # Schedule next attempt
            self._schedule_reconnection(actual_delay)
            
            # Increase delay for next attempt using exponential backoff
            self.current_delay = min(self.current_delay * self.config.backoff_factor, self.config.max_delay)
    ...
    def _start_health_check_timer(self) -> None:
        ...
        self.health_check_timer = threading.Timer(self.config.health_check_interval, self._perform_health_check)
    ...
    def _perform_health_check(self) -> None:
        ...
        try:
            ...
            if self.state == ReconnectionState.IDLE:
                logger.debug(f"Performing health check for connection {self.connection_id}")
                is_healthy = self.health_check_callback()
                
                if not is_healthy:
                    logger.warning(f"Health check failed for connection {self.connection_id}, initiating reconnection")
                    self.connection_lost()
        except Exception as e:
            logger.error(f"Error during health check for {self.connection_id}: {str(e)}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_break_until": self.circuit_break_until.isoformat() if self.circuit_break_until else None,
