...
class DashboardMetrics:
    ...
    def update_trade(self, trade: Dict[str, Any]):
        ...
        # Update win rate
        winning_trades = sum(1 for t in self.trades_history if t.get("realized_pnl", 0) > 0)
    ...
    def update_system_health(
        ...
        self.system_health["latency"].append({
            "timestamp": timestamp,
            "value": latency
        })
    ...
    def _calculate_performance_metrics(self):
        ...
        self.cumulative_returns = [{
            "timestamp": timestamp,
            "value": value
        } for timestamp, value in zip(cumulative_returns.index, cumulative_returns.values)]
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        current_portfolio_value = (
            ...
            if self.portfolio_value_history else 0.0
        )
        
        current_cash = (
            self.cash_history[-1]["value"] 
            if self.cash_history else 0.0
        )
        ...
        return {
            ...
            "performance": {
                "daily_return": self.daily_returns[-1]["value"] if self.daily_returns else 0.0,
                "cumulative_return": self.cumulative_returns[-1]["value"] if self.cumulative_returns else 0.0,
            ...
            "system": {
                "cpu_usage": self.system_health["cpu_usage"][-1]["value"] if self.system_health["cpu_usage"] else 0.0,
                "memory_usage": self.system_health["memory_usage"][-1]["value"] if self.system_health["memory_usage"] else 0.0,
                "latency": self.system_health["latency"][-1]["value"] if self.system_health["latency"] else 0.0,
                "errors": sum(item["value"] for item in self.system_health["errors"][-100:]) if self.system_health["errors"] else 0
            }
        }
...
class DashboardServer:
    ...
    def _create_layout(self):
        ...
        navbar = dbc.Navbar(
            dbc.Container([
                dbc.Row([
                    ...
                    dbc.Col(dbc.NavbarBrand("AlphaMind Trading Dashboard", className="ms-2")),
                ], align="center"),
                dbc.Row([
                    dbc.Col(
                        dbc.Nav([
        ...
        orders_section = html.Div([
            ...
            dbc.Card([
                ...
                dbc.CardBody(
                    html.Div(id="active-orders-table")
                )
            ], className="mb-4"),
            dbc.Card([
    ...
    def _register_callbacks(self, app):
        ...
        def update_overview_cards(n):
            ...
            portfolio_change_class = "text-success" if daily_return >= 0 else "text-danger"
        ...
        def update_positions_table(n):
            ...
            for symbol, position in positions.items():
                ...
                unrealized_pnl_percent = unrealized_pnl / (quantity * avg_price) if quantity * avg_price > 0 else 0
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['cumulative_return']:.2%}")
                            )
                        ]),
                        width=6
                    ),
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['volatility']:.2%}")
                            )
                        ]),
                        width=6
                    )
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Card([
                            dbc.CardHeader("Sharpe Ratio"),
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['sharpe_ratio']:.2f}")
                            )
                        ]),
                        width=6
                    ),
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['sortino_ratio']:.2f}")
                            )
                        ]),
                        width=6
                    )
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['max_drawdown']:.2%}")
                            )
                        ]),
                        width=6
                    ),
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['performance']['win_rate']:.2%}")
                            )
                        ]),
                        width=6
                    )
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Card([
                            ...
                            dbc.CardBody(
                                html.H4(f"{summary['risk']['expected_shortfall']:.2%}")
                            )
                        ]),
                        width=6
                    ),
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Card([
                            dbc.CardHeader("Positions"),
                            dbc.CardBody(
                                html.H4(f"{summary['portfolio']['positions_count']}")
                            )
                        ]),
                        width=6
                    ),
                    dbc.Col(
                        dbc.Card([
                            dbc.CardHeader("Active Orders"),
                            dbc.CardBody(
                                html.H4(f"{summary['trading']['active_orders']}")
                            )
                        ]),
                        width=6
                    )
        ...
        def update_market_charts(n, symbol):
            ...
            if market_data:
                ...
                if ohlcv_data:
                    ...
                    for data_point in market_data:
                        if "channel" in data_point and data_point["channel"] == "trades":
            ...
            if orderbook_data and "bids" in orderbook_data and "asks" in orderbook_data:
                ...
                # Add bid trace
                orderbook_fig.add_trace(go.Scatter(
                    x=bids_df["price"],
        ...
        def update_market_overview(n):
            ...
            for symbol in symbols:
                ...
                price_change_class = "text-success" if price_change and price_change >= 0 else "text-danger"
        ...
        def update_active_orders_table(n):
            ...
            for order in active_orders:
                ...
                side_class = "text-success" if order.get("side") == "buy" else "text-danger"
                ...
                row = html.Tr([
                    ...
                    html.Td(f"${order.get('price', 0):,.2f}" if order.get("price") else "MARKET"),
                    html.Td(f"{order.get('filled_quantity', 0) / order.get('quantity', 1) * 100:.1f}%"),
        ...
        def update_recent_trades_table(n):
            ...
            for trade in reversed(recent_trades):
                ...
                side_class = "text-success" if trade.get("side") == "buy" else "text-danger"
                pnl_class = "text-success" if trade.get("realized_pnl", 0) >= 0 else "text-danger"
