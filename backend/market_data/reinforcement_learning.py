...
class TradingEnvironment(ABC):
    ...
    def _setup_observation_space(self) -> Dict[str, Any]:
        ...
        if self.observation_type == ObservationType.NUMERIC:
            ...
            return {
                "type": "numeric",
                "shape": (self.window_size, len(self.features) + 4),  # Features + position, balance, equity, pnl
            ...
            return {
                "type": "image",
                "shape": (len(self.features) + 4, self.window_size, 1),  # Features + position, balance, equity, pnl
                "features": self.features + ["position", "balance", "equity", "pnl"]
            }
        else:
    ...
    def reset(self) -> np.ndarray:
        ...
        if self.random_start and len(self.data) > self.window_size + self.max_steps:
            self.start_idx = random.randint(0, len(self.data) - self.window_size - self.max_steps - 1)
        else:
            self.start_idx = 0
    ...
    def step(self, action: Union[int, float, np.ndarray]) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        """
    ...
    def _parse_action(self, action: Union[int, float, np.ndarray]) -> Dict[str, Any]:
        ...
        if self.action_space_type == ActionSpace.DISCRETE:
            ...
            if isinstance(action, tuple):
                ...
                raise ValueError(f"Invalid action format for mixed action space: {action}")
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if self.current_step > 0:
            positions = np.array([self.history["positions"][-self.window_size:] + [self.position]])
            balances = np.array([self.history["balances"][-self.window_size:] + [self.balance]])
            equities = np.array([self.history["equities"][-self.window_size:] + [self.equity]])
    ...
    def _calculate_reward(self) -> float:
        ...
        if self.reward_function == RewardFunction.RETURNS:
            ...
            returns = np.array(self.history["returns"]) / np.array(self.history["equities"][:-1])
    ...
    def render(self, mode: str = "human"):
        ...
        if mode == "human":
            ...
            print(f"Total Return: {(self.equity - self.initial_balance) / self.initial_balance:.2%}")
            print("-" * 50)
    ...
    def __init__(
        ...
        if len(input_shape) == 2:
            ...
            self.network = nn.Sequential(
                ...
                nn.MaxPool2d(kernel_size=2, stride=2),
                nn.Flatten(),
                nn.Linear(64 * (input_shape[1] // 4) * (input_shape[2] // 4), hidden_size),
                nn.ReLU(),
                nn.Linear(hidden_size, num_actions)
            )
...
class DQNAgent:
    ...
    def __init__(
        ...
        # Create networks
        self.policy_net = DQNNetwork(input_shape, num_actions, hidden_size).to(self.device)
        self.target_net = DQNNetwork(input_shape, num_actions, hidden_size).to(self.device)
...
class PPOAgent:
    ...
    def __init__(
        ...
        # Create network
        self.network = ActorCritic(input_shape, action_dim, hidden_size, action_range).to(self.device)
    ...
    def select_action(
        ...
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            action, log_prob, value = self.network.get_action(state_tensor, deterministic=not training)
...
class RLTrader:
    ...
    def create_environment(
        ...
        if features is None:
            ...
            if len(available_cols) > 0:
                ...
                features = [col for col in numeric_cols if not any(time_col in col.lower() for time_col in ["date", "time", "timestamp"])]
    ...
    def create_agent(self) -> Union[DQNAgent, PPOAgent]:
        ...
        # Get input shape
        if self.observation_type == ObservationType.NUMERIC:
            input_shape = (self.window_size, len(self.features) + 4)  # Features + position, balance, equity, pnl
        elif self.observation_type == ObservationType.IMAGE:
            input_shape = (len(self.features) + 4, self.window_size, 1)  # Features + position, balance, equity, pnl
        else:
            raise ValueError(f"Unsupported observation type: {self.observation_type}")
    ...
    def backtest(
        ...
        while not done:
            ...
            if verbose and backtest_env.current_step % 100 == 0:
                print(f"Step {backtest_env.current_step}/{backtest_env.max_steps} | "
        ...
        results["performance"] = {
            ...
            "total_return": (results["equities"][-1] - results["equities"][0]) / results["equities"][0],
