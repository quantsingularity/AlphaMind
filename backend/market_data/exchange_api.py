...
#            "GET", "/api/v3/order", params=params, auth=True
#        )
...
#            "GET", "/api/v3/openOrders", params=params, auth=True
#        )
##         return [
##             self._parse_order_from_exchange(order_data) for order_data in orders_data
#        ]
#
##     async def get_order_history(
##         self,
...
#
##         orders_data = await self._request(
#            "GET", "/api/v3/allOrders", params=params, auth=True
#        )
##         return [
##             self._parse_order_from_exchange(order_data) for order_data in orders_data
#        ]
...
#
##         order_data = await self._request(
#            "POST", "/api/v3/order", params=params, auth=True
#        )
#
...
#                "DELETE", "/api/v3/openOrders", params=params, auth=True
#            )
##             return len(result)
##         except ExchangeAPIError as e:
...
##         ws_url = (
##             f"{self.config.get_api_url('websocket')}/stream?streams={streams_param}"
#        )
#
##         try:
...
#            # Start listening for messages
##             asyncio.create_task(
##                 self._listen_market_data_stream(ws_key, symbol, channels)
#            )
##         except Exception as e:
##             logger.error(f"Error connecting to market data stream: {e}")
#
##     async def _listen_market_data_stream(
##         self, ws_key: str, symbol: str, channels: List[str]
#    ):
#        """"""
...
##                 order_data.get("timeInForce"), TimeInForce.GTC
#            ),
...
##         if (
##             float(order_data["executedQty"]) > 0
##             and float(order_data.get("cummulativeQuoteQty", 0)) > 0
#        ):
##             order.average_price = float(order_data["cummulativeQuoteQty"]) / float(
