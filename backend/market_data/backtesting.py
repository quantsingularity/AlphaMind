...
class BacktestOrder:
    ...
    def fill(self, price: float, quantity: Optional[float] = None, time: Optional[datetime] = None):
        """
...
class Position:
    ...
    def apply_trade(self, side: OrderSide, quantity: float, price: float, commission: float = 0.0):
        """
...
class Portfolio:
    ...
    def apply_trade(self, symbol: str, side: OrderSide, quantity: float, price: float, 
...
class MarketSimulator:
    ...
    def __init__(
        ...
        elif self.slippage_model == "proportional" and "factor" not in self.slippage_params:
    ...
    def get_price_for_order(self, order: BacktestOrder, bar: pd.Series) -> float:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            if order.side == OrderSide.BUY and bar["low"] <= order.price:
                ...
                base_price = min(bar["open"], order.price)  # Can't get better than limit price
            elif order.side == OrderSide.SELL and bar["high"] >= order.price:
                # Sell limit order executed if high price >= limit price
                base_price = max(bar["open"], order.price)  # Can't get better than limit price
            else:
            ...
            if order.side == OrderSide.BUY and bar["high"] >= order.stop_price:
                ...
                if order.order_type == OrderType.STOP:
                    base_price = max(bar["open"], order.stop_price)  # Can't get better than stop price
                else:  # STOP_LIMIT
                ...
                if order.order_type == OrderType.STOP:
                    base_price = min(bar["open"], order.stop_price)  # Can't get better than stop price
                else:  # STOP_LIMIT
...
class BacktestEngine:
        ...
        for symbol, df in self.data.items():
            if not isinstance(df.index, pd.DatetimeIndex):
                if "timestamp" in df.columns:
                    ...
                    raise ValueError(f"DataFrame for {symbol} must have datetime index or timestamp column")
    ...
    def _calculate_performance_stats(self):
        ...
        if not equity_df.empty:
            ...
            if not returns_df.empty:
                ...
                total_return = (equity_df["equity"].iloc[-1] / self.portfolio.initial_cash) - 1
                annual_return = (1 + total_return) ** (252 / len(returns_df)) - 1 if len(returns_df) > 0 else 0
                ...
                if not trades_df.empty:
                    ...
                    trades_df["position"] = trades_df["side"].apply(
                        ...
                        for _, trade in group.iterrows():
                            ...
                            if trade_side == "buy":
                                ...
                                if position >= 0:
                                    ...
                                    avg_price = (position * avg_price + trade_qty * trade_price) / (position + trade_qty)
                                    position += trade_qty
                                    trade_pnls.append(-commission)
                                else:
                                    # Covering short position
                                    cover_qty = min(trade_qty, abs(position))
                                    pnl = cover_qty * (avg_price - trade_price) - commission
                                ...
                                if position <= 0:
                                    ...
                                    avg_price = (abs(position) * avg_price + trade_qty * trade_price) / (abs(position) + trade_qty)
                                    position -= trade_qty
                                    trade_pnls.append(-commission)
                                else:
                                    # Closing long position
                                    close_qty = min(trade_qty, position)
                                    pnl = close_qty * (trade_price - avg_price) - commission
                                    trade_pnls.append(pnl)
    ...
    def plot_results(self, figsize: Tuple[int, int] = (12, 8), save_path: Optional[str] = None):
        """
    ...
    def resample_data(
        ...
        for symbol, df in data.items():
            try:
                ...
                logger.info(f"Resampled {symbol} data to {interval} interval, resulting in {len(resampled)} bars")
...
def run_example_backtest():
    ...
    data = data_loader.load_from_csv({
        "AAPL": "data/AAPL.csv",
        "MSFT": "data/MSFT.csv"
    })
